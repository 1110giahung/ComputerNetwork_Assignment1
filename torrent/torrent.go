...
func Create(path string) (torrentPath string, err error) {
	...
	// New torrent information to be added
	newTorrentInfo := map[string]string{
		"FilePath":   path,
	...
	filePath := "torrent_info.json"
	var torrentList []map[string]string
	...
	if _, err := os.Stat(filePath); err == nil {
		...
		if json.Unmarshal(data, &torrentList) != nil {
			// If the file is not a valid array, initialize it as an empty list
			torrentList = []map[string]string{}
		}
	}

	// Check for duplicates
	found := false
	for _, existing := range torrentList {
		if existing["InfoHash"] == newTorrentInfo["InfoHash"] {
			found = true
			break
		}
	}
	if !found {
		torrentList = append(torrentList, newTorrentInfo)
	}

	// Write updated list back to torrent_info.json
	jsonData, err := json.MarshalIndent(torrentList, "", "  ")
	if err != nil {
		return "", err
	}
...
<<<<<<< HEAD:ComputerNetwork_Ass1/torrent/torrent.go
...
>>>>>>> 08c2e09d171dc6aaee478bcff82895bc0cf7eeb6:torrent/torrent.go
}
